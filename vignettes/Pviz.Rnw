...
Some example data are available in the data package \texttt{PEP.db}. Frequency
...
The peptide collections in \texttt{PEP.db} contain reference sequence as 
...
pt<-ProbeTrack(sequence = restab$peptide, intensity = restab$group2, 
               probeStart = restab$start, cex=0.2)
